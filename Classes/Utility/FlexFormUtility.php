...
class FlexFormUtility implements \TYPO3\CMS\Core\SingletonInterface
{
    ...
    public function authorize($PA, $fObj)
    {
        ...
        try {
            ...
            $GLOBALS['TYPO3_DB']->exec_UPDATEquery('sys_file_storage', 'uid = ' . $PA['row']['uid'],
                ['configuration' => GeneralUtility::array2xml_cs($config, 'T3FlexForms')]);
            $PA['item'] = '<div style="font-weight: bold; color: green; padding: 10px;">' . LocalizationUtility::translate($localLangPath . 'driverConfiguration.authCode.success',
                    '') . '</div>';
        } catch (\Exception $e) {
        }

        $PA['item'] = '<ol>
			<li>' . LocalizationUtility::translate($localLangPath . 'driverConfiguration.authCode.tutorial1',
                '') . ' <a target="_blank" href="' . $webAuth->start() . '">' . LocalizationUtility::translate($localLangPath . 'driverConfiguration.authCode.tutorial1_linkText',
                '') . '</a></li>
